...
| Layer | Members |
|---|---|
| core | deploy-guard, typecheck-on-write, shellcheck-on-write, bash-portability-on-write, pylint-on-write, contract-lock, file-ownership (tier-gated), wiki system (pages, change proposals, execution log, freshness ledger), scenarios + coverage schemas |
| engine | convergence driver (Workflow), wave execution (Workflow), tier routing, wiring/verification agents, quality-gate (thin, non-blocking under minimal) |
...
### M-2: Engine port to Workflow
Convergence loop first (all three modes on the uniform harness contract), then wave execution (Wave-0 contracts, parallel implementers via worktree isolation where available, serial wiring/verification). Markdown drivers labeled `fallback`, frozen. Exit: metrics "engine determinism", "circuit-breaker parity", "protocol corpus reduction" pass.
...
- No wiki/change-proposal ceremony slimming — worth doing, separate roadmap.
